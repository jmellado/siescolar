...
class Fotografias_controller extends CI_Controller {
	...
	public function acudientes()
	{
		...
		$this->template->load('roles/rol_administrador_vista', 'fotografias/acudientes_vista');
	}
	...
	public function mostraracudientes(){
		...
		$data = array(
			...
			'acudientes' => $this->fotografias_model->buscar_acudiente($id,$inicio,$cantidad),

		    'totalregistros' => count($this->fotografias_model->buscar_acudiente($id)),
	...
	public function cargar_fotografiaAC(){
        ...
        if ($this->form_validation->run() == FALSE){
			...
			//si el fchero es subido correctamente
			if ($this->upload->do_upload($foto_acudiente)) {
