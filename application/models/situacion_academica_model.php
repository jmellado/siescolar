		...
		for ($i=0; $i < count($criterios); $i++) {
			...
			if ($codigo_criterio == 3) {
				...
				if ($Total_AsigEspReprob == $Total_AsigEsp) {
					...
					$id_crit = 1;
					$validacion = $this->situacion_academica_model->validar_criterio_asignado($ano_lectivo,$id_grado,$id_crit);

					if ($validacion == true) {

						$crit = $this->situacion_academica_model->obtener_criterio_asignado($ano_lectivo,$id_grado,$id_crit);
