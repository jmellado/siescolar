...
function inicio(){
	...
	llenarcombo_cursosE($("#jornadaI").val());


	$("#btn_exportar_archivo").click(function(event){

    	if($("#form_exportar_notas").valid()==true){

    		$("#form_exportar_notas").submit();
       	}
       	else{
...
function llenarcombo_cursosE(jornada){
	...
	$.ajax({
		url:base_url+"exportar_notas_controller/llenarcombo_cursos",
		type:"post",
...
function llenarcombo_asignaturasE(id_curso){
	...
	$.ajax({
		url:base_url+"exportar_notas_controller/llenarcombo_asignaturas",
		type:"post",
