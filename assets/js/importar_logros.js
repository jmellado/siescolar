...
function inicio(){
	...
	$("#form_importar_logros").submit(function (event) {
		
		event.preventDefault();
		var formData = new FormData($("#form_importar_logros")[0]);

		if($("#form_importar_logros").valid()==true){

			$.ajax({

				url:$("#form_importar_logros").attr("action"),
				type:$("#form_importar_logros").attr("method"),
				data:formData,
				cache:false,
				contentType:false,
				processData:false,   
				success:function(respuesta) {
					...
					else if(respuesta==="logrosincorrectos"){
						
						toastr.warning('Los Logros Ingresados Son Incorrectos.', 'Success Alert', {timeOut: 3000});
...
function llenarcombo_cursosI(jornada){
	...
	$.ajax({
		url:base_url+"importar_logros_controller/llenarcombo_cursos",
		type:"post",
...
function llenarcombo_asignaturasI(id_curso){
	...
	$.ajax({
		url:base_url+"importar_logros_controller/llenarcombo_asignaturas",
		type:"post",
