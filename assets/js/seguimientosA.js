...
function inicio(){
	...
	llenarcombo_profesoresSG();
	llenarcombo_tipos_causalesSG();
	llenarcombo_acciones_pedagogicasSG();
	mostrarseguimientos("",1,5);


	// este metodo permite enviar la inf del formulario
	$("#form_seguimientos").submit(function (event) {
		//validar()
		event.preventDefault(); //evita que se ejecute la accion del boton del formulario
		if($("#form_seguimientos").valid()==true){

			$.ajax({

				url:$("#form_seguimientos").attr("action"),
				type:$("#form_seguimientos").attr("method"),
				data:$("#form_seguimientos").serialize(),   //captura la info de la cajas de texto
				success:function(respuesta) {

					//alert(""+respuesta);
					if (respuesta==="registroguardado") {
						
						toastr.success('Seguimiento Registrado Satisfactoriamente.', 'Success Alert', {timeOut: 3000});
						$("#form_seguimientos")[0].reset();
						limpiar_combosSG();

					}
					else if(respuesta==="registronoguardado"){
						
						toastr.error('Seguimiento No Registrado.', 'Success Alert', {timeOut: 3000});
						

					}
					else if(respuesta==="seguimientoyaexiste"){
						
						toastr.warning('Seguimiento Ya Registrado.', 'Success Alert', {timeOut: 3000});
	...
	$("#id_profesorSG").change(function(){
    	id_profesor = $(this).val();
    	llenarcombo_cursos_profesorSG(id_profesor);
    	$("#asignaturas_seguimientos1 select").html("");
    ...
    $("#id_cursoSG").change(function(){
    	id_curso = $(this).val();
    	id_profesor = $("#id_profesorSG").val();
    	llenarcombo_asignaturas_profesorSG(id_profesor,id_curso);
    	$("#estudiantes_seguimientos1 select").html("");
    ...
    $("#btn_cancelar_seguimiento").click(function(event){
       	$("#form_seguimientos")[0].reset();
       	var validator = $("#form_seguimientos").validate();
        validator.resetForm();
        limpiar_combosSG();
    });


    $("#form_seguimientos").validate({
    ...
    $("#buscar_seguimiento").keyup(function(event){
		...
		mostrarseguimientos(buscar,1,valorcantidad);
    ...
    $("body").on("click", ".paginacion_seguimiento li a", function(event){
    	...
    	if(numero_pagina !="#" && numero_pagina != "javascript:void(0)"){
			...
			mostrarseguimientos(buscar,numero_pagina,valorcantidad);
		}	
    ...
    $("#modal_actualizar_seguimiento").on('hidden.bs.modal', function () {
        ...
        var validator = $("#form_seguimientos_actualizar").validate();
        validator.resetForm();
    });
    ...
    $("#btn_cerrar_seguimiento").click(function(event){
		...
		if($("#id_tipo_causalseleSG").val() !="" && $("#id_causalseleSG").val() !="" && $("#fecha_causalseleSG").val() !="" && $("#descripcion_situacionseleSG").val() !="" && $("#id_accion_pedagogicaseleSG").val() !="" && $("#descripcion_accionseleSG").val() !="" && $("#compromiso_estudianteseleSG").val() !="" && $("#observacionesseleSG").val() !=""){
			...
			if(confirm("Esta Seguro De Cerrar Este Seguimiento.?")){
				cerrar_seguimiento(id_seguimiento);
			}
		}
		else{
...
function llenarcombo_profesoresSG(){

	$.ajax({
		url:base_url+"seguimientos_controller/llenarcombo_profesores",
		type:"post",
		success:function(respuesta) {
				...
				$("#profesores_seguimientos1 select").html(html);
		}
...
function llenarcombo_cursos_profesorSG(valor){
	...
	$.ajax({
		url:base_url+"seguimientos_controller/llenarcombo_cursos_profesor",
		type:"post",
...
function llenarcombo_asignaturas_profesorSG(valor,valor2){
	...
	$.ajax({
		url:base_url+"seguimientos_controller/llenarcombo_asignaturas_profesor",
		type:"post",
...
function llenarcombo_estudiantesSG(valor){
	...
	$.ajax({
		url:base_url+"seguimientos_controller/llenarcombo_estudiantes",
		type:"post",
		data:{id_curso:valor},
		success:function(respuesta) {
...
function llenarcombo_tipos_causalesSG(){
	...
	$.ajax({
		url:base_url+"seguimientos_controller/llenarcombo_tipos_causales",
		type:"post",
		success:function(respuesta) {
...
function llenarcombo_causalesSG(valor,valor2){
	...
	$.ajax({
		url:base_url+"seguimientos_controller/llenarcombo_causales",
		type:"post",
...
function llenarcombo_acciones_pedagogicasSG(){
	...
	$.ajax({
		url:base_url+"seguimientos_controller/llenarcombo_acciones_pedagogicas",
		type:"post",
		success:function(respuesta) {
...
function limpiar_combosSG(){

	$("#cursos_seguimientos1 select").html("");
    $("#asignaturas_seguimientos1 select").html("");
	$("#estudiantes_seguimientos1 select").html("");
	$("#causales_seguimientos1 select").html("");
...
function mostrarseguimientos(valor,pagina,cantidad){

	$.ajax({
		url:base_url+"seguimientos_controller/mostrarseguimientos",
		type:"post",
...
function actualizar_seguimiento(){
	...
	$.ajax({
		url:base_url+"seguimientos_controller/modificar",
		type:"post",
		...
		success:function(respuesta) {
				...
				if (respuesta==="registroactualizado") {
					...
					toastr.warning('No Se Puede Modificar La Información De Este Seguimiento; El Año Lectivo En El Que Fue Registrado, Se Encuentra Cerrado.', 'Success Alert', {timeOut: 3000});
...
function cerrar_seguimiento(id_seguimiento){

	$.ajax({
		url:base_url+"seguimientos_controller/cerrar_seguimiento",
		type:"post",
		...
		success:function(respuesta) {
				...
				if (respuesta==="seguimientocerrado") {
					...
					toastr.success('Seguimiento Cerrado Satisfactoriamente.', 'Success Alert', {timeOut: 3000});

				}
				else if(respuesta==="seguimientonocerrado"){
					
					toastr.error('Seguimiento No Cerrado.', 'Success Alert', {timeOut: 3000});
					
				}
				else if(respuesta==="seguimientoyacerrado"){
					
					toastr.warning('Ya Se Encuentra Cerrado Este Seguimiento.', 'Success Alert', {timeOut: 3000});
