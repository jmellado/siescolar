			...
			$.ajax({
				...
				success:function(respuesta) {
					...
					if (respuesta==="registroguardado") {
						...
						toastr.success('Seguimiento Disiciplinario Guardado Satisfactoriamente.', 'Success Alert', {timeOut: 5000});
						...
						toastr.error('Seguimiento Disiciplinario No Guardado.', 'Success Alert', {timeOut: 5000});
		...
		success:function(respuesta) {
				...
				if (registros.seguimientos.length > 0) {
					...
					for (var i = 0; i < registros.seguimientos.length; i++) {
						html +="<tr><td>"+[i+1]+"</td><td style='display:none'>"+registros.seguimientos[i].id_seguimiento+"</td><td style='display:none'>"+registros.seguimientos[i].id_curso+"</td><td>"+registros.seguimientos[i].nombre_grado+" "+registros.seguimientos[i].nombre_grupo+" "+registros.seguimientos[i].jornada+"</td><td style='display:none'>"+registros.seguimientos[i].id_asignatura+"</td><td style='display:none'>"+registros.seguimientos[i].nombre_asignatura+"</td><td style='display:none'>"+registros.seguimientos[i].id_estudiante+"</td><td>"+registros.seguimientos[i].nombres+" "+registros.seguimientos[i].apellido1+" "+registros.seguimientos[i].apellido2+"</td><td style='display:none'>"+registros.seguimientos[i].id_tipo_causal+"</td><td>"+registros.seguimientos[i].tipo_causal+"</td><td style='display:none'>"+registros.seguimientos[i].id_causal+"</td><td style='display:none'>"+registros.seguimientos[i].causal+"</td><td style='display:none'>"+registros.seguimientos[i].descripcion_situacion+"</td><td>"+registros.seguimientos[i].fecha_causal+"</td><td><a class='btn btn-success' href="+registros.seguimientos[i].id_seguimiento+"><i class='fa fa-edit'></i></a></td><td style='display:none'><button type='button' class='btn btn-danger' value="+registros.seguimientos[i].id_seguimiento+"><i class='fa fa-trash'></i></button></td></tr>";
					};
	...
	$.ajax({
		...
		success:function(respuesta) {
				...
				if (respuesta==="registroactualizado") {
					...
					toastr.success('Seguimiento Disciplinario Actualizado Satisfactoriamente.', 'Success Alert', {timeOut: 3000});

				}
				else if(respuesta==="registronoactualizado"){
					
					toastr.error('Seguimiento Disciplinario No Actualizado.', 'Success Alert', {timeOut: 3000});
